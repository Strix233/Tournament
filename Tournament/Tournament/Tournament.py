import json
SoloEvents = []#stating all arrays at begining of the program
GroupEvents = []
Teams = []
Individuals = []
def SaveData():
...
def RunSoloEvents():
    ...
    while Repeated!=False:
            for i in range(0,5):
                    ...
                    Twentyth=VerifySoloInput("Who got 20th place")
                    if len({First, Second, Third, Fourth,Fifth,Sixth,Seventh,Eighth,Nineth,Tenth,Eleventh,Twelveth,Thirtheenth,Fourteenth,Fifteenth,Sixteenth,Seventeenth,Eighteenth,Nineteenth,Twentyth}) == 20:
                        Repeated=False
                        for j in range(0,20):
...
def Menu():
    while True:
        ...
        Choice = Choice.upper()
        if Choice in ["A", "B", "C", "D","S","L"]: #Esnuser no incorrect input is given, prevents program crash
            return Choice
        else:
